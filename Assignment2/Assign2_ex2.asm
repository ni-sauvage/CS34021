...
rec_call:
    pop r10                         ; Access local variable
    sub rsp, 40                     ; Allocate 40 bytes of shadow space on stack (16 byte aligned as (40 + 8)%16 == 0), for 4 parameters
    call my_sum_4                   ; Recursively call my_sum_4
    add rax, r10                    ; Add result of my_sum_4 to local variable
    add rsp, 40                     ; Deallocate 40 bytes of shadow space
    push r10                        ; store local variable
clean_stack:
...
printing:                       
    ...
    sub rsp, 40                     ; Sub 40 from rsp (must have 32 bytes, but must keep stack 16 byte aligned, so sub 40, which will be 48 with RIP)
    call print                      ; Call procedure to do system calls to print our number
    add rsp, 40                     ; Add 40 bytes to rsp (move stack pointer to original position) while keeping aligned
